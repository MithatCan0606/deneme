...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get(consts.mute_hammer_role) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));

  var user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));

    if(!user) return message.reply("Lütfen Sessize Alınacak Bir Kullanıcıyı Etiketleyin 1y = Yıl 1d Gün 1h = saat 1m = Dakika 1s = saniye ").then(m => m.delete(5000));
    if(user.hasPermission("ADMINISTRATOR")) return message.reply("Yöneticileri Susturamazsın!!").then(m => m.delete(5000));
    
    //var muterole = message.guild.roles.find(`name`, `muted`);
    var muterole = message.guild.roles.get(consts.voice_mute_role)
  ...
  setTimeout(function(){
    ...
    .setDescription(`${user} adlı Kullanıcının **Mute** süresi doldu`)
   .setFooter(`${client.user.tag}` , `${client.user.displayAvatarURL}`)
  .setTimestamp()  

    let sChannel = message.guild.channels.get(consts.mute_log_channel)
    if(!sChannel) return
    sChannel.send(embed)
  },1000)
    },  ms(muteTime));
